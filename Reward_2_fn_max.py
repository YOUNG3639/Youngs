...
def find_periods_for_conditions():
    ...
    # t-3부터 비교 시작, 마지막까지 비교 가능
    for t in range(3, len(actual_gdp)):
        # 경우 1: 실업률 하락 & 산업생산 증가율 상승 & 인플레이션 상승
        if (unemployment_rate[t-3] > unemployment_rate[t-2] > unemployment_rate[t-1] and
            industrial_production_index[t-3] < industrial_production_index[t-2] < industrial_production_index[t-1] and
            inflation[t-3] < inflation[t-2] < inflation[t-1]):
